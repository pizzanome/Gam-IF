...
function printData(data) {
    ...
    PREFIX owl: <http://www.w3.org/2002/07/owl#>
        ...
        SELECT *
        WHERE {
        <http://dbpedia.org/resource/Nintendo_Switch> rdfs:label ?name.
    ...
    /*
     PREFIX owl: <http://www.w3.org/2002/07/owl#>
        ...
        SELECT ?name ?dev ?directeur ?publisher ?description (GROUP_CONCAT(DISTINCT ?dates;SEPARATOR=";") AS ?date) (GROUP_CONCAT(DISTINCT ?genres;SEPARATOR=";") AS ?genre) (GROUP_CONCAT(DISTINCT ?plateformes;SEPARATOR=";") AS ?plateforme)
        WHERE {
        <http://dbpedia.org/resource/Mario's_Game_Gallery> foaf:name ?name; dbp:genre ?genres; dbo:releaseDate ?dates; dbp:developer ?dev; dbo:publisher ?publisher; dbo:abstract ?description; dbp:platforms ?plateformes.
        FILTER(langMatches(lang(?description),"FR"))
        OPTIONAL{<http://dbpedia.org/resource/Mario's_Game_Gallery> dbp:director ?directeur}
        }
     */
